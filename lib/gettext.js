...
Gettext.prototype._getTranslation = function(domain, msgid, context, count){
    var d = domain || this._textdomain;
    domain = d ? d.toLowerCase().trim() || '';
...
Gettext.prototype.getKeys = function(domain, context) {
    domain = domain ? domain.toLowerCase().trim() || '';
    var textdomain = this._domains[domain];
    return textdomain ? textdomain.getKeys(context) : [];
};
